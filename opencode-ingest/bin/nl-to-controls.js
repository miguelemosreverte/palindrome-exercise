...
export function buildPrompt(query, controls) {
  ...
  // Prepare city display: group by country
  const cityByCountry = {};
  controls.cities.forEach(c => {
    const parts = c.split('|');
    const country = parts[0];
    if (!cityByCountry[country]) cityByCountry[country] = [];
    if (parts.length > 1) cityByCountry[country].push(parts[1]);
  });
  const cityStr = Object.entries(cityByCountry).map(([country, cities]) =>
...
FILTERS:
  ...
  city (hierarchical Country|City): ${cityStr}
...
export function validateAndFix(result, controls) {
  ...
  // Fix city: hierarchical matching (Argentina, Argentina|Córdoba, etc.)
  if (result.city) {
    const cityStr = String(result.city).split(',')[0].trim();
    // Try exact match first
    let fixed = controls.cities.find(c => c.toLowerCase() === cityStr.toLowerCase());
    if (!fixed) {
      // Try matching just the city part: "Córdoba" → "Argentina|Córdoba"
      fixed = controls.cities.find(c => c.split('|').pop().toLowerCase() === cityStr.toLowerCase());
    }
    if (!fixed) {
      // Try matching country part: "Argentina" → "Argentina" (country level)
      fixed = controls.cities.find(c => c.split('|')[0].toLowerCase() === cityStr.toLowerCase());
      if (!fixed) fixed = controls.cities.find(c => c.toLowerCase().startsWith(cityStr.toLowerCase()));
    }
