...
function buildGraph(records, columns, showcaseResults) {
  ...
  // Build facet dropdowns — handles pipe-delimited hierarchical values
  const dropdown = (id, label, values) => {
    const entries = Object.entries(values).sort((a,b) => b[1]-a[1]);

    // Check if values have pipe hierarchy (like city: Argentina|Córdoba)
    const hasPipes = entries.some(([v]) => v.includes('|'));

    let opts;
    if (hasPipes) {
      // Group by country, show cities indented
      const countries = {};
      const countryTotals = {};
      entries.forEach(([v, c]) => {
        const parts = v.split('|');
        const country = parts[0];
        const city = parts.length > 1 ? parts.slice(1).join('|') : null;
        if (!countries[country]) countries[country] = [];
        if (city) countries[country].push({ city, value: v, count: c });
        else countryTotals[country] = c;
      });

      opts = '';
      // Sort countries by total count (sum of all their cities + country-only)
      const countryList = Object.keys(countries).sort((a, b) => {
        const ta = (countryTotals[a] || 0) + countries[a].reduce((s, c) => s + c.count, 0);
        const tb = (countryTotals[b] || 0) + countries[b].reduce((s, c) => s + c.count, 0);
        return tb - ta;
      });

      for (const country of countryList) {
        const total = (countryTotals[country] || 0) + countries[country].reduce((s, c) => s + c.count, 0);
        // Country-level option (matches ALL people in this country)
        opts += `<option value="${escHtml(country)}">${escHtml(country)} (${total})</option>`;
        // City-level options
        countries[country].sort((a, b) => b.count - a.count).forEach(c => {
          opts += `<option value="${escHtml(c.value)}">⠀${escHtml(c.city)} (${c.count})</option>`;
        });
      }
    } else {
      opts = entries.map(([v, c]) => `<option value="${escHtml(v)}">${escHtml(v)} (${c})</option>`).join('');
    }
  ...
  return `<section class="layout-section">
    ...
    <div class="chart-row" id="live-charts">
      <div class="chart-card" id="domain-chart"></div>
      <div class="chart-card" id="seniority-chart"></div>
...
(function() {
  ...
  function applyFilters() {
    ...
    for (const [facet, value] of Object.entries(activeFilters)) {
      ...
      if (facet === 'seniority_score_min') {
        ...
        // Hierarchical: "Argentina" matches "Argentina", "Argentina|Córdoba", etc.
        filtered = filtered.filter(r => {
          const v = r[facet] || '';
  ...
  function renderCharts(filtered) {
    // ── Domain chart: if only 1 domain, decompose into skill subcategories ──
    const domains = {};
    filtered.forEach(r => { const d = r.domain; if (d) domains[d] = (domains[d]||0)+1; });
    ...
    if (domainEl) {
      ...
      if (domainEntries.length <= 1) {
        // Decompose: show skill subcategories instead
        const subcats = {};
        filtered.forEach(r => {
          try { JSON.parse(r.skills_normalized || '[]').forEach(s => {
            if (!s.path) return;
        ...
        items = Object.entries(subcats).sort((a,b) => b[1]-a[1]).slice(0, 8);
        domainEl.querySelector('.chart-title')?.remove();
        domainEl.innerHTML = '<h4 class="chart-title">Specializations</h4>' + renderHBars(items);
      } else {
        items = domainEntries.slice(0, 8);
        domainEl.innerHTML = '<h4 class="chart-title">Domain</h4>' + renderHBars(items);
      }
    }
    ...
    const skills = {};
    filtered.forEach(r => {
      try { JSON.parse(r.skills_normalized || '[]').filter(s => s.confidence >= 0.8).forEach(s => {
        const leaf = s.path?.split('|').pop();
        if (leaf) skills[leaf] = (skills[leaf]||0)+1;
      }); } catch {}
    });
    const skillEl = document.getElementById('skills-chart');
    if (skillEl) {
      const sorted = Object.entries(skills).sort((a,b) => b[1]-a[1]).slice(0, 20);
      const max = sorted[0]?.[1] || 1;
      const min = sorted.at(-1)?.[1] || 0;
      const range = max - min || 1;
      skillEl.innerHTML = '<h4 class="chart-title">Top Skills</h4><div class="cloud-tags">' + sorted.map(([label, value]) => {
        const ratio = (value - min) / range;
        const size = (0.7 + ratio * 0.8).toFixed(2);
        const opacity = (0.5 + ratio * 0.5).toFixed(2);
        return '<span class="cloud-tag" style="font-size:' + size + 'rem;opacity:' + opacity + '" title="' + label + ': ' + value + '">' + label + '</span>';
      }).join(' ') + '</div>';
    }
  }
  ...
  function renderHBars(items) {
    ...
    return items.map(([label, value]) => {
      ...
      return '<div class="hbar-row"><span class="hbar-label">' + label + '</span><div class="hbar-track"><div class="hbar-fill" style="width:' + pct + '%;background:' + BAR_COLOR + '"></div></div><span class="hbar-count">' + value + '</span></div>';
    }).join('');
  }
...
const CSS_LAYOUTS = `
  ...
  @media (max-width: 640px) { .hbar-label { width: 70px; font-size: 0.7rem; } }
