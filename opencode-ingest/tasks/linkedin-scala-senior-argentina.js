...
/**
 * Scala Senior Engineers in Argentina — LinkedIn
 *
 * Headhunter task: finds senior Scala developers in Argentina.
 ...
 */
export default class LinkedInScalaSeniorArgentinaScraper extends Scraper {
  ...
  sources() {
    const queries = [
      'scala%20senior%20engineer',
      'scala%20developer%20senior',
      'scala%20lead%20engineer',
      'scala%20architect',
      'senior%20backend%20scala',
    ];
  ...
  async next() {
    ...
    try {
      ...
      wf(pjoin(htmlDir, `${String(pageNum).padStart(3, '0')}-search.html`), await page.content());
      ...
      for (const profileUrl of profileUrls) {
        ...
        try {
          ...
          wf(pjoin(htmlDir, `${String(pageNum).padStart(3, '0')}-profile-${slug}.html`), await page.content());
          ...
          const record = await page.evaluate(({url, name}) => {
            ...
            // Detect Scala relevance from skills and headline
            const allText = (title + ' ' + headline + ' ' + skills.join(' ')).toLowerCase();
            const scalaRelevant = allText.includes('scala') || allText.includes('akka') ||
              allText.includes('spark') || allText.includes('play framework') ||
              allText.includes('functional programming') || allText.includes('jvm');
          ...
          if (record.name && record.name.length > 2) {
            records.push(record);
            const tag = record.scalaRelevant ? '🎯' : '  ';
            console.log(`  ${tag} ${record.name} — ${record.title} @ ${record.company}`);
          }
        } catch (err) {
