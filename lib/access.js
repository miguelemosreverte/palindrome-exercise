...
async function ensureApprovedPaymentAccess(ref, payment) {
  const next = {
    ...
    demo_requests: Number(payment.demo_requests || 0),
  };
...
function summarizePaidAccess(records) {
  return records.reduce(
    (acc, record) => {
      ...
      acc.requests += Number(record.demo_requests || 0);
...
async function consumePaidAccess(identity, estimatedCostUsd) {
  ...
  await patchPayment(usable.ref, {
    ...
    demo_requests: Number(usable.demo_requests || 0) + 1,
    last_demo_at: Date.now(),
  });

  const updated = hydrated.map((record) =>
    record.ref === usable.ref ? { ...record, spent_estimated_usd: nextSpent, demo_requests: Number(record.demo_requests || 0) + 1 } : record
  );
...
async function consumeAnonymousAccess(anonId, estimatedCostUsd) {
  ...
  const nextSpent = Number((Number(current.spent_estimated_usd || 0) + estimatedCostUsd).toFixed(6));
  ...
  if (nextSpent > Number(current.credit_limit_usd || FREE_DEMO_CREDIT_USD) || nextRequests > Number(current.requests_limit || FREE_DEMO_REQUESTS)) {
    return { ok: false, summary: current };
  }
