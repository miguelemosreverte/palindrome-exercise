...
function getData(collection, taskId) {
  if (taskId) {
    return db.prepare('SELECT * FROM collected_data WHERE collection LIKE ? AND task_id = ? ORDER BY created_at DESC').all(collection + '%', taskId);
  }
  return db.prepare('SELECT * FROM collected_data WHERE collection LIKE ? ORDER BY created_at DESC LIMIT 100').all(collection + '%');
}
